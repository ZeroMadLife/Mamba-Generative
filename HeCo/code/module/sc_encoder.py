...
class inter_att(nn.Module):
    ...
    def forward(self, embeds):
        ...
        beta = self.softmax(beta)  # type-level attention ap as
...
class intra_att(nn.Module):
    ...
    def forward(self, nei, h, h_refer):
        #  p a s nei_a[4019, ] nei_s[4019, ]
        #  args.type_num = [4019, 7167, 60]  # the number of every node type p a s
        nei_emb = F.embedding(nei, h)  #  每一行代表p取了哪些a或者s邻居
        # F.embeding代表的是这个a节点的邻居的嵌入向量 torch.Size([4019, 7, 64])
...
class Sc_encoder(nn.Module):
    def __init__(self, hidden_dim, sample_rate, nei_num, attn_drop):
        ...
        #  同类型 内聚合 不同类型 类别间聚合 有点像分层聚类
        self.intra = nn.ModuleList([intra_att(hidden_dim, attn_drop) for _ in range(nei_num)])  # nei_num 有多少种邻居类型

...
class GRACE(torch.nn.Module):
    ...
    def semi_loss(self, z1: torch.Tensor, z2: torch.Tensor,z3,z4,pair_dict):
        f = lambda x: torch.exp(x / self.tau)
        refl_sim = f(self.sim(z1, z1))
        between_sim = f(self.sim(z1, z2))
        neg13 = f(self.sim(z1, z3))
        neg14 = f(self.sim(z1, z4))
        pair_dict.to('cuda')
        pos1 = refl_sim.gather(1, pair_dict).sum(dim=1)
        pos2 = between_sim.gather(1, pair_dict).sum(dim=1)
        neg1 = neg13.gather(1, pair_dict).sum(dim=1)
        neg2 = neg14.gather(1, pair_dict).sum(dim=1)
        return -torch.log((between_sim.diag()+pos1+pos2) / (refl_sim.sum(1) + between_sim.sum(1) - refl_sim.diag()-pos1-pos2+neg1+neg2))


    def loss(self, z1, z2,z3,z4, pair_dict,mean: bool = True, batch_size: Optional[int] = None):
        h1 = self.projection(z1)
        h2 = self.projection(z2)
        h3 = self.projection(z3)
        h4 = self.projection(z4)

        l1 = self.semi_loss(h1, h2,h3,h4,pair_dict)
        l2 = self.semi_loss(h2, h1,h3,h4,pair_dict)
...
class NewEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor, R = [1,2], final = False):
        if final == False:
            K2 = np.random.randint(1, 3)
            K1 = np.random.randint(2, 4)
        if final:
            K1 = 2
            K2 = 2
        feat = x
